...
setupModules <- function(name, paths, modules, inProject, useGit = getOption("SpaDES.project.useGit", FALSE),
  ...
  if (missing(modules)) {
    ...
    if (usingGit || ( length(anyFailedGH) ) ) {
      ...
      browser()
      gitSplit <- splitGitRepo(gitModules)
      gitSplit <-try(Require::invertList(gitSplit), silent = TRUE)
      if (is(gitSplit, "try-error"))
        stop("Did you specify the gitModules correctly? Is this correct:\n",
             paste(gitModules, collapse = "\n"))

      gitSplit <- unique(gitSplit) # there could be several gitModules within the same repository; only clone once
